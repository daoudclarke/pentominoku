...
test('simple sudoku can be solved', () => {
  let solver = new Solver([rowRestriction, columnRestriction, boxRestriction]);
  ...
  let possible = solver.getPossible(fixedPoints);
...
test('row restriction works', () => {
  let solver = new Solver([rowRestriction]);
...
test('row elimination works', () => {
  let solver = new Solver([columnRestriction, rowRestriction]);
...
test('column restriction works', () => {
  let solver = new Solver([columnRestriction]);
  let fixedPoints = [[4, 5], [0, 3]];
...
test('box restriction works', () => {
  let solver = new Solver([boxRestriction]);
...
test('test knights move', () => {
  let solver = new Solver([knightsMoveRestriction]);
...
test('test kings move', () => {
  let solver = new Solver([kingsMoveRestriction]);
  let fixedPoints = [[0, 1], [33, 5]];
...
test('test orthogonal consecutive restriction', () => {
  let solver = new Solver([orthogonalConsecutiveRestriction]);
  let fixedPoints = [[0, 1], [33, 5]];
...
test('test miracle sudoku', () => {
  const solver = new Solver([rowRestriction, columnRestriction, boxRestriction, knightsMoveRestriction,
